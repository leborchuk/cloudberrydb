CREATE EXTENSION anser_test CASCADE;
...
-- SQL-function round trip through the live services: producer_begin -> publish
...
SELECT anser.producer_begin(10, 1, 1, 'svc_roundtrip', 1) AS begin_ok;
SELECT anser.publish(10, 1, 1, 'svc_roundtrip', '\x6162'::bytea, false) AS publish_ok;
SELECT encode(anser.consume_wait(10, 1, 1, 'svc_roundtrip'), 'escape') AS payload;
...
-- after anser.timeout_ms, so the waiting consumer is delivered a cancel and
-- consume_wait returns NULL.
SELECT anser.producer_begin(11, 1, 1, 'svc_timeout', 1) AS begin_ok;
SELECT anser_test_state(11, 1, 1, 'svc_timeout') AS state_after_begin;
SELECT anser.consume_wait(11, 1, 1, 'svc_timeout') IS NULL AS consume_cancelled;
