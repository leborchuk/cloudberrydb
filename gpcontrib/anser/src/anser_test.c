...
anser_test_consume(PG_FUNCTION_ARGS)
{
	 ...
	 * gather service's stale-channel sweep after anser.timeout_ms, which wakes
...
anser_test_dsm_free_on_timeout(PG_FUNCTION_ARGS)
{
	...
	SetConfigOption("anser.timeout_ms", "100", PGC_USERSET, PGC_S_SESSION);
	...
	PG_TRY();
	{
		...
		if (part != NULL && AnserProducerBegin(&key, 5, GetUserId(), superuser()))
		{
			 ...
			 */
			pg_usleep(200000L);		/* 200 ms > anser.timeout_ms (100 ms) */
	...
	PG_FINALLY();
	{
		SetConfigOption("anser.timeout_ms", saved_timeout,
...
/*
 * Open a loopback connection to our coordinator and fire anser.consume_wait
