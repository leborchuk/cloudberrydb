 ...
 * explicit anser.max_channels overrides it entirely.
 */
 ...
 *
 * A remote producer backend (anser.publish) hands one part to the gather
 ...
 * Consumer wait table.
 *
 * A remote consumer backend (anser.consume_wait) registers a slot and blocks
...
/*
 * The named tranche requested in _PG_init, resolved in AnserShmemInit().  NULL
 * until then, which is one of the things AnserInitialized() checks.
 */
LWLock	   *AnserChannelLock = NULL;
LWLock	   *AnserRingLock = NULL;
...
AnserShmemSize(void)
{
	...
	size = add_size(size, AnserAuthShmemSize());

	return size;
}

/*
 * Create (postmaster) or attach to (EXEC_BACKEND child) the Anser shared state.
 * Called from the shmem_startup_hook, after _PG_init has requested both the
 * space and the LWLock tranche.
 */
void
AnserShmemInit(void)
{
	...
	locks = GetNamedLWLockTranche(ANSER_LWLOCK_TRANCHE);
	AnserChannelLock = &locks[0].lock;
	AnserRingLock = &locks[1].lock;
				  ...
				  Oid caller_role, bool caller_is_super)
{
	 ...
	 * anser.timeout_ms) for registration instead of failing open at once.
 ...
 * READY within anser.timeout_ms.  Cancellation is whole-dataset:
 ...
 *
 * When anser.max_channels is set explicitly (> 0) it wins.  Otherwise the map
...
{
	return gp_anser_enable && AnserChannelLock != NULL && AnserCtl != NULL &&
		AnserChannelHash != NULL && AnserWaitTable != NULL;
}
