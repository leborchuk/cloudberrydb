 ...
 *
 * anserfuncs.c
 *	  SQL functions that expose the Anser network transport.
 *
 ...
 * by opening an ordinary libpq connection to the QD and calling these functions
 * (created by anser--1.0.sql in schema "anser"); all real work happens in the
 * gather and send background services.  A producer announces itself with
 * anser.producer_begin(), streams parts with anser.publish(), and a consumer
 * blocks in anser.consume_wait() until the send service delivers its payload
 * (or cancels it).
 *
 * Access control: these functions are intentionally left with the default
 * EXECUTE grant to PUBLIC.  The network transport connects to the QD as the
 * *query's own* role, so restricting them to superusers -- or revoking them
 * from PUBLIC -- would silently disable runtime filtering for every
 * non-superuser query (it would fail open to unfiltered execution).
 *
 * The (session id, command count, condition) key cannot be derived server-side:
 * each call runs in a fresh coordinator backend the segment opened over
 ...
 *
 * IDENTIFICATION
 *	  gpcontrib/anser/src/anserfuncs.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
PG_FUNCTION_INFO_V1(anser_producer_begin);
PG_FUNCTION_INFO_V1(anser_publish);
PG_FUNCTION_INFO_V1(anser_consume_wait);

static bool anser_build_key(int32 gp_session_id, int32 gp_command_count,
									int32 condition_id, text *condition_key_text,
									AnserChannelKey *key);

/*
 * anser_producer_begin(ssid, ccnt, cond_id, cond_key, expected_producers)
 *
 ...
 * within anser.timeout_ms the gather service cancels the whole dataset.
 */
Datum
anser_producer_begin(PG_FUNCTION_ARGS)
{
	...
	if (!anser_build_key(gp_session_id, gp_command_count, condition_id,
								 condition_key, &key))
		PG_RETURN_BOOL(false);
...
/*
 * anser_publish(ssid, ccnt, cond_id, cond_key, payload, cancelled)
 *
 * Hand one part to the gather service and block for its ACK.  expected_producers
 * is not repeated here: anser.producer_begin already stamped it on the
 * channel, so we pass 0 to leave it unchanged.
 */
Datum
anser_publish(PG_FUNCTION_ARGS)
{
	int32		gp_session_id = PG_GETARG_INT32(0);
	int32		gp_command_count = PG_GETARG_INT32(1);
	int32		condition_id = PG_GETARG_INT32(2);
...
/*
 * anser_consume_wait(ssid, ccnt, cond_id, cond_key) -> bytea
 *
 ...
 */
Datum
anser_consume_wait(PG_FUNCTION_ARGS)
{
	int32		gp_session_id = PG_GETARG_INT32(0);
	int32		gp_command_count = PG_GETARG_INT32(1);
	int32		condition_id = PG_GETARG_INT32(2);
...
/*
 * anser_build_key(ssid, ccnt, cond_id, cond_key, key)
 *
 ...
 */
static bool
anser_build_key(int32 gp_session_id, int32 gp_command_count,
						int32 condition_id, text *condition_key_text,
						AnserChannelKey *key)
{
