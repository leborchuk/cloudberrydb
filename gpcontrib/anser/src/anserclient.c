 ...
 * anser.* SQL functions.  When the QD supplied a session token (carried
 * in the plan), the connection authenticates with it via the anser.conn
 ...
 *
 * IDENTIFICATION
 *	  gpcontrib/anser/src/anserclient.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
 ...
 *
 * When a session token is given, the connection carries the anser.conn=true
 ...
 * AnserConnCheckPassword, installed as the core custom-auth hook) -- so no
 * pg_hba entry for the
...
/*
 * Run a bool-returning anser.* function.  Returns 1 (true), 0 (false), or -1
							...
							uint32 expected_producers)
{
	...
	return anser_client_exec_bool(conn,
								  "SELECT anser.producer_begin($1::int4, $2::int4, $3::int4, $4::text, $5::int4)",
								  5, values, NULL, NULL);
}
						  ...
						  const void *payload, Size payload_len, bool cancelled)
{
	...
	return anser_client_exec_bool(conn,
								  "SELECT anser.publish($1::int4, $2::int4, $3::int4, $4::text, $5::bytea, $6::bool)",
 ...
 * anser.consume_wait there unwinds and its wait slot is reaped.
 */
static PGresult *
					   ...
					   Size *payload_len, bool *cancelled, const char *token)
{
	...
	PG_TRY();
	{
		res = anser_client_wait_result(conn,
									   "SELECT anser.consume_wait($1::int4, $2::int4, $3::int4, $4::text)",
