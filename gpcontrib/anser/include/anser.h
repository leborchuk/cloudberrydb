...
/*
 * The two LWLocks of the "anser" named tranche, requested in _PG_init and
 * resolved in AnserShmemInit: AnserChannelLock guards the channel map, the
 * consumer wait table and the session-token hash; AnserRingLock guards the
 * inbound submission queue.
 */
extern LWLock *AnserChannelLock;
extern LWLock *AnserRingLock;

#define ANSER_LWLOCK_TRANCHE	"anser"
#define ANSER_NUM_LWLOCKS		2
...
/*
 * The session-token hash owned by anserauth.c; folded into the sizing and
 * setup above so all Anser shared state is requested in one place.
 */
extern Size AnserAuthShmemSize(void);
extern void AnserAuthShmemInit(void);

/*
 * Effective channel-map size (anser.max_channels, or its auto-sizing from
 ...
 * Network-path API.
 *
 * These entry points back the anser.* SQL functions that remote
...
/*
 * Background-service entry points.  The two mains are resolved by name from
 * this library (bgw_function_name), so they must be exported.
 */
extern PGDLLEXPORT void AnserGatherServiceMain(Datum main_arg);
extern PGDLLEXPORT void AnserSendServiceMain(Datum main_arg);
 ...
 * AnserGetOrCreateSessionToken at plan time; the coordinator backend accepting
 * the connection calls AnserConnClaims/AnserConnCheckPassword, which _PG_init
 * installs as the core custom-authentication hooks (see libpq/auth.h).
 */
...
extern bool AnserConnClaims(struct Port *port);
extern bool AnserConnCheckPassword(struct Port *port, const char *passwd);
